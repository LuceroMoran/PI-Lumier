...
function evaluate(session, modelUrl, actions, currentActionModel, userInput, onContextCreationHandler) {
    if (!modelUrl) {
    ...
    lib.dialog('Evaluate', new builder.SimpleDialog(function (session, args) {
        ...
        function next() {
            ...
            if (actionModel.status === Status.ContextSwitch && args.response === true) {
                ...
                operation = evaluate(session, modelUrl, actions, actionModel);
            } else if (args && args.response && actionModel.currentParameter) {
                // try evaluate new parameter
                operation = evaluate(session, modelUrl, actions, actionModel, args.response);
            } else {
        ...
        try {
            validate(action.schema, actionModel.parameters,
                ...
                (completeParameters) => {
                    ...
                    action.fulfill(session, completeParameters, (fulfillResult) => {
